...
\begin{table}[h]
	...
	\label{tb:input}
\end{table}
A Tabela \ref{tb:input} lista todos os valores escolhidos para resolução numérica do modelo das batalhas. Note que a condição CFL $\left(\frac{D\Delta t}{(\Delta x)^2} < 0.194 < 1/2\right)$ foi atendida, logo, para os parâmetros escolhidos, o método explícito não apresenta instabilidades.
...
\begin{figure}[ht]
	\centering
	\includegraphics[scale=0.6]{figs/battle1.png}
	\caption{}
	\label{fig:battle1}
\end{figure}
